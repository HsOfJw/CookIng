cc.Class({
    ...
    onLoad() {
        ...
        this.com.loadTexture(this.node, ["houjing"], "jpg_cooking_bg01");
        this.com.loadTexture(this.node, ["qianjing", "zhuanqiang"], "png_zhuanqiang");
        this.com.loadTexture(this.node, ["qianjing", "chuju_jiazi_xia"], "png_chuju_jiazi_xia");
        this.com.loadTexture(this.node, ["qianjing", "chuju_dingbanzuo"], "png_chuju_dingbanzuo");
        this.com.loadTexture(this.node, ["qianjing", "chuju_jiazi"], "png_chuju_jiazi");
        this.com.loadTexture(this.node, ["qianjing", "chuju_chouti_xia"], "png_chuju_chouti_xia");
        this.com.loadTexture(this.node, ["qianjing", "chuju_chouti_shang"], "png_chuju_chouti_shang");
        this.com.loadTexture(this.node, ["qianjing", "chuju_jiazi_you"], "png_chuju_jiazi_you");
        this.com.loadTexture(this.node, ["qianjing", "chuju_dingban"], "png_chuju_dingban");
        this.com.loadTexture(this.node, ["qianjing", "chuju_shaokao"], "png_chuju_shaokao");
        this.com.loadTexture(this.node, ["qianjing", "chuju_caiban"], "png_chuju_caiban");
        this.com.loadTexture(this.node, ["qianjing", "front_img_l"], "png_front_img_l");
        this.com.loadTexture(this.node, ["qianjing", "front_img_r"], "png_front_img_r");
        ...
        if (this.com.isSave) {
            ...
            this.httpUtils._instance.httpPost(url, JSON.stringify(params), function (data) {
                console.log("/user/getLoginShareCooking接收到数据", data);
    ...
    initFriend: function () {
        ...
        //drinkDispenser.getComponent('LyWorkbench').reset(this.com.saveData.shopItem["210"], this.com.saveData.shopItem["110"], false);
    ...
    initDesk: function () {
        ...
        this.initLayerLock("Grill", "Grill_lock", "220");
        this.initLayerLock("Grill_long", "Grill_long_lock", "230");
        this.initLayerLock("cabinet", "cabinet_lock", "221");
        this.initLayerLock("cabinet_long", "cabinet_long_lock", "231");
        this.initLayerLock("tray", "tray_lock", "210");
    ...
    initWorkbenchIcon: function () {
        var rl = this.com.saveData.shopItem["120"];
        var rou = this.com.loadTexture(this.node, ["qianjing", "btnMeat"], "png_" + rl + "_s");
        if (rl)
            rou.x = -147;
        else
            rou.x = 10000;

        var xl = this.com.saveData.shopItem["130"];
        var xiangchang = this.com.loadTexture(this.node, ["qianjing", "btnSausage"], "png_" + xl + "_s");
        if (xl)
            xiangchang.x = -255;
        else
            xiangchang.x = 10000;

        var ml = this.com.saveData.shopItem["121"];
        var mianbao = this.com.loadTexture(this.node, ["qianjing", "btnBread"], "png_" + ml + "_s");
        if (ml)
            mianbao.x = 56;
        else
            mianbao.x = 10000;

        var cl = this.com.saveData.shopItem["131"];
        var changmianbao = this.com.loadTexture(this.node, ["qianjing", "btnStreusel"], "png_" + cl + "_s");
        if (cl)
            changmianbao.x = 197;
        else
            changmianbao.x = 10000;

        var gl = this.com.saveData.shopItem["132"];
        var guojiang = this.com.loadTexture(this.node, ["qianjing", "btnJam"], "png_" + gl + "_s");
        if (gl)
            guojiang.x = 460;
        else
            guojiang.x = 10000;

        var sl = this.com.saveData.shopItem["122"];
        var shengcai = this.com.loadTexture(this.node, ["qianjing", "btnLettuce"], "png_" + sl + "_s");
        if (sl)
            shengcai.x = 525;
        else
            shengcai.x = 10000;

        var kl = this.com.saveData.shopItem["110"];
        this.com.loadTexture(this.node, ["keleji"], "png_" + kl + "_d");

        var recycle = this.com.loadTexture(this.node, ["qianjing", "recycle"], "png_chuju_lajixiang");
        recycle.x = 364;
    },

    canAddTray: function () {
        var lyTrip = this.node.getChildByName("tray");
        for (var i = 0; i < this.com.getMachineLevel("210"); i++) {
    ...
    addTray: function () {
        ...
        for (var i = 0; i < this.com.getMachineLevel("210"); i++) {
            ...
            if (!node.children.length) {
                ...
                dishes.getComponent('LyDishes').init(true, this.com.saveData.shopItem["110"], 1.0);
                dishes.parent = node;
    ...
    canAddBarbecue: function (isLong) {
        ...
        var level = this.com.getMachineLevel("220");
        if (isLong) {
            lyGrill = this.node.getChildByName("Grill_long");
            level = this.com.getMachineLevel("230");
        }
    ...
    canAddCabinet: function (isLong) {
        ...
        var level = this.com.getMachineLevel("221");
        if (isLong) {
            lyCabinet = this.node.getChildByName("cabinet_long");
            level = this.com.getMachineLevel("231");
        }
        for (var i = 0; i < level; i++) {
        ...
        char.runAction(cc.spawn(
            cc.sequence(
                ...
                cc.callFunc(function () {
                    ...
                    if (self.com.saveData.month == 1) {
                        if (self.m_peopleNum == 1 && self.com.isSave && (self.com.saveData.newbie == null || self.com.saveData.newbie <= 4105)) {
                            self.pauseGame();
                            self.com.saveData.newbie = null;
                            self.com.showGuide(4101, self.node, self.m_newbie);
                        } else if (self.m_peopleNum == 3 && self.com.saveData.newbie == 4106 && self.com.isSave && self.com.saveData.newbie <= 4205) {
                            self.pauseGame();
                            self.com.showGuide(4201, self.node, self.m_newbie);
                            self.guidePause();
                            self.com.showGuide(4202, self.node, self.m_newbie);
                        } else if (self.m_peopleNum == 5 && self.com.saveData.newbie == 4205 && self.com.isSave && self.com.saveData.newbie <= 4305) {
                            self.pauseGame();
                            self.com.showGuide(4301, self.node, self.m_newbie);
                        }
                    }
                    if (self.com.saveData.month == 3 && self.m_peopleNum == 1 && self.com.isSave && (self.com.saveData.newbie == null || self.com.saveData.newbie < 4601)) {
                        self.pauseGame();
                        self.com.showGuide(4601, self.node, self.m_newbie);
                    }
                    ...
                    newNode.runAction(cc.sequence(
                        ...
                        cc.callFunc(function (target, data) {
                            if (ndFood) {
                                ...
                                if (self.com.saveData.month == 1 && self.com.saveData.newbie == 4105 && self.com.isSave) {
                                    self.com.showGuide(self.com.saveData.newbie + 1, self.node, self.m_newbie);
                                }
    ...
    unlockDishes: function () {
        for (var i = 0; i < this.com.cfgFood.length; i++) {
            ...
            if (food.unlock == this.com.fluctuate && food.unlockmonth == this.com.saveData.month) {
                var find = false;
                ...
                if (!find) {
                    ...
                    for (var j = 0; j < 5; j++) {
                        ...
                        if (food["food" + j] == 130 && shopItem["130"] == null) {
                            shopItem["130"] = 1301;
                            show_dlg_array.push(1301);
                        }
                        if (food["food" + j] == 131 && shopItem["131"] == null) {
                            shopItem["131"] = 1311;
                            show_dlg_array.push(1311);
                        }
                        if (food["food" + j] == 122 && shopItem["122"] == null) {
                            shopItem["122"] = 1221;
                            show_dlg_array.push(1221);
                        }
                        if (food["food" + j] == 132 && shopItem["132"] == null) {
                            shopItem["132"] = 1321;
                            show_dlg_array.push(1321);
                        }

                        this.com.setComData("shopItem", shopItem);

                        if (food["food" + j] == 130 && shopItem["230"] == null) {
                            shopItem["230"] = 2301;
                            this.upgradeUnLock("Grill_long_lock", "230");
                        }
                        if (food["food" + j] == 131 && shopItem["231"] == null) {
                            shopItem["231"] = 2311;
                            this.upgradeUnLock("cabinet_long_lock", "231");
                        }
                        this.initWorkbenchIcon();
                    }
    ...
    pauseGame: function () {
        ...
        var lyGrill = this.node.getChildByName("Grill");
        for (var i = 0; i < this.com.getMachineLevel("220"); i++) {
        ...
        for (var i = 0; i < this.com.getMachineLevel("230"); i++) {
    ...
    BtnMeat: function (event, coustEvent) {
        if (this.canAddBarbecue(false)) {
            this.addBarbecue(this.com.saveData.shopItem["220"], this.com.saveData.shopItem["120"], false);
        }
        if (this.com.saveData.month == 1 && this.m_peopleNum == 1 && this.com.saveData.newbie == 4102 && this.com.isSave) {
            var lyGrill = this.node.getChildByName("Grill");
            for (var i = 0; i < this.com.getMachineLevel("220"); i++) {
                var nd = lyGrill.getChildByName("node_" + i);
                if (nd.children[0]) {
    ...
    BtnSausage: function (event, coustEvent) {
        if (this.canAddBarbecue(true)) {
            this.addBarbecue(this.com.saveData.shopItem["230"], this.com.saveData.shopItem["130"], true);
        }
    },

    BtnBread: function (event, coustEvent) {
        if (this.canAddCabinet(false)) {
            this.addCabinet(this.com.saveData.shopItem["121"], false);
        }
        if (this.com.saveData.month == 1 && this.m_peopleNum == 1 && this.com.saveData.newbie == 4103 && this.com.isSave) {
            this.com.showGuide(this.com.saveData.newbie + 1, this.node, this.m_newbie);
        }
    },

    BtnStreusel: function (event, coustEvent) {
        if (this.canAddCabinet(true)) {
            this.addCabinet(this.com.saveData.shopItem["131"], true);
        }
    },

    BtnLettuce: function (event, coustEvent) {
        if (this.canCompose(this.com.saveData.shopItem["221"], this.com.saveData.shopItem["122"], false)) {
            this.compose(this.com.saveData.shopItem["221"], this.com.saveData.shopItem["122"], false);
        }
    },

    BtnJam: function (event, coustEvent) {
        if (this.canCompose(this.com.saveData.shopItem["231"], this.com.saveData.shopItem["132"], true)) {
            this.compose(this.com.saveData.shopItem["231"], this.com.saveData.shopItem["132"], true);
        }
    },
    ...
    upgradeItem: function (event) {
        ...
        this.upgradeUnLock("Grill_lock", "220");
        this.upgradeUnLock("Grill_long_lock", "230");
        this.upgradeUnLock("cabinet_lock", "221");
        this.upgradeUnLock("cabinet_long_lock", "231");
        this.upgradeUnLock("tray_lock", "210");
    ...
    helpFriend: function (event) {
        ...
        drinkDispenser.children[0].getComponent('LyWorkbench').reset(this.com.saveData.shopItem["210"], this.com.saveData.shopItem["110"], false);
